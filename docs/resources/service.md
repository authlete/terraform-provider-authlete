...
- `organization_id` (Number) The numeric ID of the organization the service belongs to. Requires replacement if changed.
...
- `api_server_id` (Number) The numeric ID of the API server to create the service on. Official SDKs inject
...
- `user_code_charset` (String) The character set for end-user verification codes (`user_code`) for Device Flow. must be one of ["BASE20", "NUMERIC"]
...
- `verified_claims_validation_schema_set` (String) The verified claims validation schema set. must be one of ["standard", "standard+id_document"]
...
```terraform
import {
  to = authlete_service.my_authlete_service
  id = 0
}
```
