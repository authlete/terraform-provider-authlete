...
The client secret can be set on Authlete server overwriting the secret on server side (either the random generated when creating the client or existing secret on server side).
The provider will not check the complexity of the secret, nor the min or max size of it.
