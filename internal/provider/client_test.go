...
func TestClientCreateImport(t *testing.T) {
	if testedAuthleteVersionNotSmaller("3.0") {
		t.Skip("Skipping test as Authlete version is above 3.0")
		return
	}
	openid := "openid"
	profile := "profile"
...
func TestClientCreateImport30(t *testing.T) {
	if testedAuthleteVersionNotBigger("3.0") {
		t.Skip("Skipping test as Authlete version is below 3.0")
		return
	}
	openid := "openid"
	profile := "profile"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: stateSimpleClientState30,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("authlete_client.client1", "client_id_alias", "terraform_client"),
					resource.TestCheckResourceAttrSet("authlete_client.client1", "client_id"),
					resource.TestCheckResourceAttrSet("authlete_client.client1", "client_secret"),
