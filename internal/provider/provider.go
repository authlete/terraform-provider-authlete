...
import (
	"context"
	"crypto/tls"
	authletefunctions "github.com/authlete/terraform-provider-authlete/internal/provider/functions"
...
func (p *AuthleteProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	// customer's own IdP and supplies the cluster's apiServerId so customers do
	// not have to look the number up. A no-op on the shared cloud with no
	// override set.
	...
	httpClient.Transport = NewIdpRoutingTransport(idpHost, apiServerID, httpClient.Transport)
